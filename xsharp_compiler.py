...
class Environment:
	...
	def define_symbol(
		...
		var_type: Literal["const", "sub", "var", "array"],
	):
		res = CompileResult()

		if symbol in self.symbols:
			return res.fail(
				CompilationError(None, None, f"Symbol {symbol} is already defined.", 3)
			)
		if var_type == "const":
	...
	def get_symbol(self, iden: Identifier | str):
		...
		if symbol not in self.symbols:
			return res.fail(
				CompilationError(
					iden.start_pos, iden.end_pos, f"Symbol {symbol} is undefined.", 4
				)
			)
...
class Compiler:
	...
	def compile(self, ast: Statements, remove_that_one_line: bool = False):
		...
		if not ast.body + ast.subroutine_defs:
			return res.success(["HALT"])
	...
	def no_visit_method(self, node, env: Environment):
		return CompileResult().fail(
			CompilationError(
				node.start_pos,
				node.end_pos,
				f"Undefined visit{type(node).__name__} method.",
				0,
			)
		)
	...
	def allocate(self, node) -> CompileResult:
		...
		if not self.available_registers:
			return res.fail(
				CompilationError(
					node.start_pos, node.end_pos, "Allocation limit exceeded!", 1
				)
			)

		free_register: int = tuple(self.available_registers - self.allocated_registers)[
			0
		]
	...
	def load_immediate(self, value: int | str, forced: bool = False) -> None:
		...
		if self.address != value or forced or '.' in self.instructions[-1]:
			self.write(f"LDIA {value}")

		if isinstance(value, int):
			self.address = value
		else:
			if value.startswith("r"):
				self.address = int(value[1:])
			else:
				self.address = value
	...
	def visitIntLiteral(self, node: IntLiteral, env: Environment) -> CompileResult:
		if node.value in self.KNOWN_VALUES:
			self.write(f"COMP {node.value} D")
		else:
	...
	def visitBinaryOperation(
		self, node: BinaryOperation, env: Environment
	) -> CompileResult:
		res = CompileResult()
		op_map = {
			"ADD": "D+M",
		...
		if isinstance(left, int) and isinstance(right, int):
			...
			value = eval(expr.replace("D", f"{left}").replace("M", f"{right}"))
			if isinstance(value, bool):
				value = -int(value)  # Ensures true = -1, false = 0
			if operation == "SUB":
				value = -value  # Subtraction fix
		...
		if operation == "MUL":
			...
			self.load_immediate(reg_left)
			self.comment("Multiplier")
			self.write("COMP M D")
			self.load_immediate(product)
			self.comment("Product")
	...
	def visitUnaryOperation(
		self, node: UnaryOperation, env: Environment
	) -> CompileResult:
		res = CompileResult()
		operation: str = str(node.op.token_type)
				...
				case _:
					return res.fail(
						CompilationError(
							node.op.start_pos,
							node.op.end_pos,
							f"Unsupported unary operator: {operation}",
							2,
						)
					)
	...
	def visitIdentifier(self, node: Identifier, env: Environment) -> CompileResult:
		...
		self.comment(node.symbol)
		self.write("COMP M D")
		return res.success(None)

	def visitConstDefinition(
		self, node: ConstDefinition, env: Environment
	) -> CompileResult:
		res = CompileResult()
	...
	def visitVarDeclaration(
		self, node: VarDeclaration, env: Environment
	) -> CompileResult:
		res = CompileResult()
		...
		if node.length is not None:
				...
				if node.length not in env.constants:
					return res.fail(
						CompilationError(
							node.start_pos,
							node.end_pos,
							f"Undefined constant: {node.length}",
							7,
						)
					)
				...
				if length != len(node.value.elements):
					return res.fail(
						CompilationError(
							node.start_pos,
							node.end_pos,
							f"Expected {length} elements, got {len(node.value.elements)} elements instead.",
							8,
						)
					)
			...
			res.register(
				env.define_symbol(
					node.identifier, (location, env.assign_address), "array"
				)
			)
				...
				if value not in (-1, 0):
					return res.fail(
						CompilationError(
							...
							node.value.end_pos,
							"Expected a boolean, found an integer instead.",
							5,
						)
					)
		...
		if value or node.length is not None:
			if value in self.KNOWN_VALUES:
				self.truncate(value_pos)
			self.load_immediate(location)
			self.comment(node.identifier)
			self.write(
				"COMP D M" if value not in self.KNOWN_VALUES else f"COMP {value} M"
			)
	...
	def visitAssignment(self, node: Assignment, env: Environment) -> CompileResult:
		...
		if node.identifier.symbol in env.constants:
			return res.fail(
				CompilationError(
					node.identifier.start_pos,
					node.identifier.end_pos,
					f"Cannot assign to constant {node.identifier.symbol}.",
					6,
				)
			)
		if node.identifier.symbol in env.subroutines:
			return res.fail(
				CompilationError(
					node.identifier.start_pos,
					node.identifier.end_pos,
					f"Cannot assign to subroutine {node.identifier.symbol}.",
					6,
				)
			)
		...
		self.load_immediate(address)
		self.comment(node.identifier.symbol)
	...
	def visitCForLoop(self, node: CForLoop, env: Environment) -> CompileResult:
		...
		if res.error:
			return res
		if start in self.KNOWN_VALUES:
			self.truncate(start_pos)
		self.load_immediate(address, forced=True)
		self.comment(node.identifier)
			...
			if node.array.symbol not in env.arrays:
				return res.fail(
					CompilationError(
						...
						node.array.end_pos,
						f"Undefined array: {node.array.symbol}",
						7,
					)
				)
			...
			if index < 0:
				return res.fail(
					CompilationError(
						...
						node.index.end_pos,
						"Out of bounds: index must be at least 0.",
						9,
					)
				)
				...
				if index >= len(node.array.elements):
					return res.fail(
						CompilationError(
							node.index.start_pos,
							node.index.end_pos,
							f"Out of bounds: index must be less than {len(node.array.elements)}.",
							9,
						)
					)
			else:
				...
				if index >= address - base_pointer:
					return res.fail(
						CompilationError(
							node.index.start_pos,
							node.index.end_pos,
							f"Out of bounds: index must be less than {address - base_pointer}.",
							9,
						)
					)

		if index == 0:
			self.truncate(index_pos)

		self.load_immediate(base_pointer, forced=True)
		self.comment("Base pointer")
		if index != 0:
			self.write("COMP D+A A")
		self.write("COMP M D")
	...
	def visitSubroutineDef(self, node: SubroutineDef, env: Environment):
		...
		location = env.assign_address
		res.register(
			env.define_symbol(node.name, (location, len(node.parameters)), "sub")
		)
		if res.error:
			return res
	...
	def visitCallExpression(self, node: CallExpression, env: Environment):
		...
		if (symb not in env.subroutines) and (symb not in env.native_subs):
			return res.fail(
				CompilationError(
					node.start_pos,
					node.end_pos,
					f"Symbol {symb} is not a subroutine.",
					10,
				)
			)

		args = (
			env.subroutines[symb] if symb in env.subroutines else env.native_subs[symb]
		)
		if args != len(node.arguments):
			return res.fail(
				CompilationError(
					node.start_pos,
					node.end_pos,
					f"Expected {args} arguments, got {len(node.arguments)} arguments instead.",
					11,
				)
			)
		...
		if symb in env.subroutines:
			...
			match symb:
				case "update":
					self.write("BUFR update")
				case "flip":
					self.write("BUFR move")
				case "halt":
					self.write("HALT")
				case "plot":
					...
					self.comment("Y Port")
					if y in self.KNOWN_VALUES:
						self.truncate(pos)
						self.write(f"COMP {y} M")
					else:
						self.write("COMP D M")
					...
					if val not in (0, 1):
						return res.fail(
							CompilationError(
								node.arguments[2].start_pos,
								node.arguments[2].end_pos,
								f"Expected 0 or 1.",
								5,
							)
						)
